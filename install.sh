...
AddDevice=/dev/kfd
AddDevice=/dev/dri
# Inherit host user groups (render, video) for GPU access + explicit fallbacks
GroupAdd=keep-groups
GroupAdd=render
GroupAdd=video
# Disable SELinux label confinement — required for device access on Arch
SecurityLabelDisable=true
# Shared memory — vLLM needs much more than the 64MB podman default
ShmSize=8g
# Persistent HF model cache across container restarts
Volume=$VLLM_HF_CACHE_VOL:/root/.cache/huggingface
Environment=HF_HUB_ENABLE_HF_TRANSFER=1
Environment=VLLM_ROCM_USE_AITER=1
# vLLM serve args — all on one line to avoid quadlet line-continuation issues.
# Change --model to swap the inference target; restart the service after.
...
if [ "$OPT_AI" = true ]; then
    ...
    if [ -d "$HERMES_REPO" ]; then
        echo -e "\n${C_CYAN}[ INFO ]${RESET} Deploying Hermes dotfiles from repo..."

        # Backup existing ~/.hermes if present
        if [ -d "$HERMES_HOME" ] || [ -L "$HERMES_HOME" ]; then
            mv "$HERMES_HOME" "$BACKUP_DIR/hermes-$(date +%s)"
            printf "  -> Backed up existing ~/.hermes %-14s ${C_GREEN}[ OK ]${RESET}\n" ""
        fi

        # Copy the whole tree from the repo
        cp -r "$HERMES_REPO" "$HERMES_HOME"
        chmod 700 "$HERMES_HOME"
        printf "  -> Copied .hermes/ from repo %-18s ${C_GREEN}[ OK ]${RESET}\n" ""
        ...
        # The repo ships .env.template (no secrets). We generate the real
        # .env with runtime values injected.
        HERMES_ENV="$HERMES_HOME/.env"
        HERMES_ENV_TMPL="$HERMES_HOME/.env.template"
        if [ ! -f "$HERMES_ENV" ]; then
            if [ "$USE_VLLM_TURBOQUANT" = true ]; then
                _env_inf_key="VLLM_API_KEY=vllm"
                _env_inf_url="VLLM_BASE_URL=http://localhost:8000/v1"
            else
                _env_inf_key="OLLAMA_API_KEY=ollama"
                _env_inf_url="OLLAMA_BASE_URL=http://localhost:11434/v1"
            fi

            if [ -f "$HERMES_ENV_TMPL" ]; then
                sed \
                    -e "s|{{INFERENCE_KEY_LINE}}|$_env_inf_key|g" \
                    -e "s|{{INFERENCE_URL_LINE}}|$_env_inf_url|g" \
                    -e "s|{{AUTOBROWSE_AUTH_TOKEN}}|$AUTOBROWSE_AUTH_TOKEN|g" \
                    "$HERMES_ENV_TMPL" > "$HERMES_ENV"
            else
                # No template — write a minimal .env directly
                printf '%s\n' \
                    ...
                    "OLLAMA_CLOUD_API_KEY=" > "$HERMES_ENV"
            fi
            chmod 600 "$HERMES_ENV"
            printf "  -> Generated .env (mode 600) %-18s ${C_GREEN}[ OK ]${RESET}\n" ""
        else
            printf "  -> .env already exists, keeping it %-12s ${C_BLUE}[KEEP]${RESET}\n" ""
        fi
        rm -f "$HERMES_HOME/.env.template" 2>/dev/null

        # ── Patch config.yaml placeholders ──
        HERMES_CFG="$HERMES_HOME/config.yaml"
        if [ -f "$HERMES_CFG" ]; then
            if [ "$USE_VLLM_TURBOQUANT" = true ]; then
                _cfg_provider="vllm-turboquant"
                _cfg_model="${VLLM_MODEL:-Qwen/Qwen2.5-7B-Instruct}"
                _cfg_base_url="http://localhost:8000/v1"
                _cfg_key_env="VLLM_API_KEY"
            else
                _cfg_provider="ollama-local"
                _cfg_model="qwen2.5:7b"
                _cfg_base_url="http://localhost:11434/v1"
                _cfg_key_env="OLLAMA_API_KEY"
            fi
            sed -i \
                -e "s|{{INFERENCE_PROVIDER}}|$_cfg_provider|g" \
                -e "s|{{INFERENCE_MODEL}}|$_cfg_model|g" \
                -e "s|{{INFERENCE_BASE_URL}}|$_cfg_base_url|g" \
                -e "s|{{INFERENCE_KEY_ENV}}|$_cfg_key_env|g" \
                "$HERMES_CFG"
            chmod 600 "$HERMES_CFG"
            printf "  -> Patched config.yaml placeholders %-10s ${C_GREEN}[ OK ]${RESET}\n" ""
        fi

        # ── MCP server registration: autobrowse ──
        if [ -n "${AUTOBROWSE_AUTH_TOKEN:-}" ] && command -v hermes &>/dev/null; then
            echo "  -> Registering autobrowse MCP server with Hermes..."
            if hermes mcp add autobrowse \
                --url "http://localhost:8080" \
                --token "$AUTOBROWSE_AUTH_TOKEN" \
                --description "Stealthy headless browser (Playwright + residential proxies)" \
                2>/dev/null; then
                printf "  -> autobrowse MCP registered %-18s ${C_GREEN}[ OK ]${RESET}\n" ""
            else
                printf "  -> autobrowse MCP registration %-16s ${C_YELLOW}[WARN]${RESET}\n" ""
            fi
        fi

        # ── PATH check ──
        if ! echo "$PATH" | tr ':' '\n' | grep -qx "$HOME/.local/bin"; then
            echo -e "  -> ${DIM}Note: ~/.local/bin not on PATH. Restart terminal or: source ~/.zshrc${RESET}"
        fi
    else
        echo -e "\n${C_YELLOW}[ WARN ]${RESET} No .hermes/ directory in repo — skipping Hermes config."
        echo -e "  -> ${DIM}Create $REPO_DIR/.hermes/ with config.yaml, .env.template, and skills/${RESET}"
    fi
