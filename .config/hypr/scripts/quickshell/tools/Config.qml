...
Item {
    ...
    function hermesSave() {
        ...
        let cmd = "mkdir -p \"" + config.cacheDir + "/qs_ai_state\" && " +
                  ...
                  "printf '%s\\n%s\\n' '" + config.hermesSessionId + "' '" + config.hermesCreatedEpoch + "' > \"" + config.hermesSessionPath + "\"";
        sh(cmd);
    }

    // Start a brand-new session (new chat): clear convo + new id/timestamp.
    ...
    function hermesAppend(msg) {
        ...
        c.push(msg);
        if (c.length > 40) c = c.slice(c.length - 40);
        config.hermesConvo = c;
        config.hermesSave();
        config.hermesConvoChanged();
    }

    Component.onCompleted: hermesLoad()
}
