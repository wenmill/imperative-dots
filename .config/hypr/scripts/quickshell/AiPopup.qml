...
Item {
    ...
    property string apiBaseUrl: "http://localhost:4000"  // overridden by ai_config.json base_url
    ...
    // When Hermes is enabled, route everything through it.
    // Hermes itself decides whether the message needs a tool call or just chat.
    function needsAgent(text) {
        return window.hermesEnabled;
    }
    ...
    function fetchModels() {
        modelFetcher.command = ["bash", "-c",
            "curl -s -H 'Authorization: Bearer " + window.apiKey + "' " +
            "'" + window.apiBaseUrl + "/api/models' 2>/dev/null || echo '[]'"
        ];
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    let models = [];
                    let arr = data.data || (Array.isArray(data) ? data : []);
                    for (let i = 0; i < arr.length; i++)
                        models.push(arr[i].id || arr[i].name);
                    if (models.length > 0) window.availableModels = models;
                } catch(e) {}
            }
        }
    }
    ...
    Component.onCompleted: { window.chessInitBoard(); window.loadCache("chat"); }
    ...
    function fetchGreeting() {
        greetingFetcher.command = ["bash", "-c",
            "echo '{\"model\":\"" + window.selectedModel + "\",\"stream\":false,\"messages\":[{\"role\":\"user\",\"content\":\"Generate a short, friendly, one-sentence greeting for someone opening an AI assistant. Be creative, warm and brief. Do not use quotes around it. Do not say you are an AI.\"}]}' | " +
            "curl -s -X POST '" + window.apiBaseUrl + "/api/chat/completions' " +
            "-H 'Authorization: Bearer " + window.apiKey + "' " +
            "-H 'Content-Type: application/json' " +
            "-d @- 2>/dev/null"
        ];
        greetingFetcher.running = false;
        greetingFetcher.running = true;
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    let data = JSON.parse(this.text.trim());
                    window.greetingText = data.choices[0].message.content;
                    window.greetingFetched = true;
                } catch(e) {
                    window.greetingText = "Hey! What can I help you with?";
                    window.greetingFetched = true;
                }
            }
        }
    ...
    Timer {
        id: greetingDelay; interval: 500; repeat: false; running: true
        onTriggered: window.fetchGreeting()
    }

    // ── Send message ──
    function sendMessage(query) {
        if (query.trim() === "" || isLoading) return;
        // Route through Hermes for action-y messages
        if (window.hermesEnabled && window.needsAgent(query)) {
        ...
        let toolKeys = Object.keys(window.activeTools);
        for (let i = 0; i < toolKeys.length; i++) {
            if (window.activeTools[toolKeys[i]]) tools.push(toolKeys[i].toLowerCase().replace(/ /g, "_"));
        }
        ...
        let escaped = payload.replace(/'/g, "'\\''");
        apiCaller.command = ["bash", "-c",
            "echo '" + escaped + "' | curl -s -X POST '" + apiBaseUrl + "/api/chat/completions' " +
            "-H 'Authorization: Bearer " + apiKey + "' " +
            "-H 'Content-Type: application/json' " +
            "-d @- 2>/dev/null"
        ];
        apiCaller.running = false;
        apiCaller.running = true;
    }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    let data = JSON.parse(this.text.trim());
                    let resp = data.choices[0].message.content;
                    chatMessages.append({ role: "assistant", content: resp });
                    window.lastResponse = resp;
                    window.typeLen = 0;
                    window.saveChatState();
                } catch(e) {
                    chatMessages.append({ role: "assistant", content: "Error: Could not get response." });
                    window.lastResponse = "Error: Could not get response.";
                    window.typeLen = 0;
                    window.saveChatState();
                }
            }
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        ListView {
                            ...
                            delegate: Item {
                                ...
                                ColumnLayout {
                                    ...
                                    // Role label
                                    Text {
                                        text: model.role === "user" ? "You" : window.selectedModel
                        ...
                        // ── Model dropdown (opens upward above input) ──
                        Rectangle {
                            Layout.fillWidth: true
                            Layout.preferredHeight: visible ? Math.min(window.availableModels.length * window.s(32), window.s(180)) : 0
                            visible: window.modelDropdownOpen && window.availableModels.length > 0
                            radius: window.s(10); color: window.surface0; border.color: window.mauve; border.width: 1; clip: true
                            Behavior on Layout.preferredHeight { NumberAnimation { duration: 200; easing.type: Easing.OutQuint } }

                            ListView {
                                anchors.fill: parent; anchors.margins: window.s(4); model: window.availableModels; clip: true; spacing: window.s(2)
                                delegate: Rectangle {
                                    width: ListView.view.width; height: window.s(28); radius: window.s(6)
                                    color: mdlMa.containsMouse ? window.surface1 : (modelData === window.selectedModel ? Qt.rgba(window.mauve.r, window.mauve.g, window.mauve.b, 0.12) : "transparent")
                                    Text {
                                        anchors.left: parent.left; anchors.leftMargin: window.s(10); anchors.verticalCenter: parent.verticalCenter
                                        text: modelData; font.family: "JetBrains Mono"; font.pixelSize: window.s(11)
                                        font.weight: modelData === window.selectedModel ? Font.Bold : Font.Medium
                                        color: modelData === window.selectedModel ? window.mauve : window.text
                                        width: parent.width - window.s(20); elide: Text.ElideRight
                                    }
                                    MouseArea { id: mdlMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { window.selectedModel = modelData; window.modelDropdownOpen = false; } }
                                }
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            ColumnLayout {
                                ...
                                // Controls: [tools] ... [model ▼] [send] ... [model ▼] [send]
                                RowLayout {
                                    Layout.fillWidth: true
                                    Layout.minimumHeight: window.s(30)
                                    Layout.maximumHeight: window.s(30)
                                    ...
                                    Rectangle {
                                        ...
                                        border.color: window.toolsPopupOpen ? window.mauve : (window.activeToolCount > 0 ? window.green : "transparent"); border.width: 1
                                        Behavior on color { ColorAnimation { duration: 120 } }
                                        Text {
                                            anchors.centerIn: parent
                                            text: "+"; font.family: "JetBrains Mono"; font.weight: Font.Black; font.pixelSize: window.s(16)
                                            color: window.toolsPopupOpen ? window.mauve : (window.activeToolCount > 0 ? window.green : window.overlay0)
                                    ...
                                    // Model selector
                                    Rectangle {
                                        Layout.preferredWidth: modelLabel.implicitWidth + window.s(24)
                                        Layout.fillHeight: true
                                        radius: window.s(8)
                                        color: modelMa.containsMouse ? window.surface1 : "transparent"
                                        border.color: window.modelDropdownOpen ? window.modeColor1 : "transparent"; border.width: 1
                                        Behavior on color { ColorAnimation { duration: 120 } }
                                        ...
                                        RowLayout {
                                            ...
                                            Text {
                                                id: modelLabel
                                                text: window.selectedModel.length > 18 ? window.selectedModel.substring(0, 18) + "…" : window.selectedModel
                                                font.family: "JetBrains Mono"; font.pixelSize: window.s(10)
                                                color: window.overlay1
                                            }
                                            Text {
                                                text: window.modelDropdownOpen ? "󰅃" : "󰅀"
                                                font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(9); color: window.overlay0
                                            }
                                        }
                                        MouseArea { id: modelMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: window.modelDropdownOpen = !window.modelDropdownOpen }
                                    }
                        ...
                        // Footer
                        RowLayout {
                            Layout.fillWidth: true; spacing: window.s(6); opacity: introInput
                            Text {
                                text: window.apiBaseUrl.replace("https://", "")
                                font.family: "JetBrains Mono"; font.pixelSize: window.s(9); color: window.overlay0
                                Layout.fillWidth: true
                            }
                            Rectangle {
                                visible: chatMessages.count > 0
                                Layout.preferredWidth: clrRow.implicitWidth + window.s(14); Layout.preferredHeight: window.s(24)
                                radius: window.s(6); color: clrMa.containsMouse ? window.surface1 : "transparent"
                                Row {
                                    id: clrRow; anchors.centerIn: parent; spacing: window.s(4)
                                    Text { text: "󰃢"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(11); color: clrMa.containsMouse ? window.red : window.overlay1; anchors.verticalCenter: parent.verticalCenter }
                                    Text { text: "Clear"; font.family: "JetBrains Mono"; font.pixelSize: window.s(9); font.weight: Font.Bold; color: clrMa.containsMouse ? window.red : window.overlay1; anchors.verticalCenter: parent.verticalCenter }
                                }
                                MouseArea { id: clrMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { chatMessages.clear(); window.lastResponse = ""; window.typeLen = 0; } }
                            }
                        }
                    }
                }
