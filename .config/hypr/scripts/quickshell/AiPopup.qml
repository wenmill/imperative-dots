...
Item {
    ...
    function saveKavitaLast(seriesId, libraryId, name) {
        saveCache("kavita_last", { seriesId: seriesId, libraryId: libraryId, name: name, format: window.kavitaReadFormat });
    }
    ...
    property string chessStatus: "menu"  // menu | aisetup | seeking | playing | ended | puzzle
    ...
    property string kavitaSubMode: "ondeck"  // "ondeck" | "library" | "reading"
    ...
    property int kavitaReadFormat: 0       // 0=image,3=epub,4=pdf
    ...
    Process {
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            for (let i = 0; i < lines.length; i++) { let parts = lines[i].split("|||");
    ...
    function chessApplyMoves(movesStr) {
        ...
        for (let m = 0; m < moves.length; m++) {
            let uci = moves[m]; let ff = uci.charCodeAt(0) - 97; let fr = 8 - parseInt(uci[1]); let tf = uci.charCodeAt(2) - 97; let tr = 8 - parseInt(uci[3]);
            let fi = fr * 8 + ff; let ti = tr * 8 + tf; let piece = b[fi]; let promo = uci.length > 4 ? uci[4] : "";
            if ((piece === "P" || piece === "p") && ff !== tf && b[ti] === "") b[fr * 8 + tf] = "";
            if (piece === "K" || piece === "k") { if (tf - ff === 2) { b[fr * 8 + 5] = b[fr * 8 + 7]; b[fr * 8 + 7] = ""; } if (ff - tf === 2) { b[fr * 8 + 3] = b[fr * 8 + 0]; b[fr * 8 + 0] = ""; } }
            b[fi] = "";
            if (promo) { let isW = piece === "P"; let pm = {"q": isW?"Q":"q", "r": isW?"R":"r", "b": isW?"B":"b", "n": isW?"N":"n"}; b[ti] = pm[promo] || piece; } else { b[ti] = piece; }
        }
        window.chessBoard = b; window.chessBoardChanged();
        let moveCount = movesStr.trim().split(" ").length; window.chessMyTurn = (moveCount % 2 === 0) === window.chessIsWhite;
        let last = moves[moves.length - 1]; window.chessFromIdx = (8 - parseInt(last[1])) * 8 + (last.charCodeAt(0) - 97); window.chessToIdx = (8 - parseInt(last[3])) * 8 + (last.charCodeAt(2) - 97);
    }
    ...
    function chessSquareClicked(idx) {
        ...
        if (window.chessSelected >= 0) {
            ...
            window.chessSelected = -1; window.chessBoardChanged(); chessMakeMove(move);
        } else {
    ...
    function chessStartAi(level, minutes) {
        ...
        chessCreateProc.command = ["bash", "-c", "LT=\"" + window.lichessToken.replace(/[\"\\$`]/g,"") + "\" curl -s -X POST 'https://lichess.org/api/challenge/ai' -H \"Authorization: Bearer $LT\" -d 'level=" + level + "&clock.limit=" + (minutes * 60) + "&clock.increment=2' 2>/dev/null"];
        chessCreateProc.running = false; chessCreateProc.running = true;
    }
    function chessSeekGame(minutes) {
        window.chessStatus = "seeking"; window.chessInitBoard(); window.chessSelected = -1; window.chessResult = "";
        chessCreateProc.command = ["bash", "-c", "LT=\"" + window.lichessToken.replace(/[\"\\$`]/g,"") + "\" curl -s --max-time 120 -X POST 'https://lichess.org/api/board/seek' -H \"Authorization: Bearer $LT\" -d 'time=" + minutes + "&increment=2&rated=false' 2>/dev/null || echo '{}'"];
        chessCreateProc.running = false; chessCreateProc.running = true;
    }
    function chessMakeMove(move) {
        chessMoveProc.command = ["bash", "-c", "LT=\"" + window.lichessToken.replace(/[\"\\$`]/g,"") + "\" curl -s -X POST 'https://lichess.org/api/board/game/" + window.chessGameId.replace(/[^a-zA-Z0-9]/g,"") + "/move/" + move.replace(/[^a-h0-9qrbn]/g,"") + "' -H \"Authorization: Bearer $LT\" 2>/dev/null"];
        chessMoveProc.running = false; chessMoveProc.running = true;
    }
    function chessResign() {
        Quickshell.execDetached(["bash", "-c", "LT=\"" + window.lichessToken.replace(/[\"\\$`]/g,"") + "\" curl -s -X POST 'https://lichess.org/api/board/game/" + window.chessGameId.replace(/[^a-zA-Z0-9]/g,"") + "/resign' -H \"Authorization: Bearer $LT\" 2>/dev/null"]);
    }
    ...
    function chessFetchPuzzle() {
        ...
        chessPuzzleProc.command = ["bash", "-c",
            "LT=\"" + window.lichessToken.replace(/[\"\\$`]/g,"") + "\" " +
            "curl -s 'https://lichess.org/api/puzzle/daily' " +
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            try {
                ...
                if (window.chessPuzzleFen) {
                    ...
                    // In puzzles, determine side from FEN (active color field)
                    let parts = window.chessPuzzleFen.split(" ");
                    window.chessIsWhite = parts.length > 1 ? parts[1] === "w" : true;
    ...
    function chessProcessEvent(eventText) {
        try {
            ...
            if (ev.type === "gameFull") {
                ...
                window.chessIsWhite = (ev.white && ev.white.id && ev.white.name) ? ev.white.name !== "" : true;
                if (ev.white && ev.white.aiLevel) window.chessIsWhite = false;
                else if (ev.black && ev.black.aiLevel) window.chessIsWhite = true;
                window.chessOpponent = window.chessIsWhite ? (ev.black ? (ev.black.name || ev.black.aiLevel ? "Stockfish L" + ev.black.aiLevel : "Opponent") : "Opponent") : (ev.white ? (ev.white.name || "Opponent") : "Opponent");
                window.chessStatus = "playing";
                if (ev.state && ev.state.moves !== undefined) { if (ev.state.moves === "") { window.chessInitBoard(); window.chessMyTurn = window.chessIsWhite; } else { window.chessApplyMoves(ev.state.moves); } } else { window.chessInitBoard(); window.chessMyTurn = window.chessIsWhite; }
                if (ev.state && ev.state.status && ev.state.status !== "started" && ev.state.status !== "created") { window.chessStatus = "ended"; window.chessResult = ev.state.status; window.saveChessState(); }
            } else if (ev.type === "gameState") {
                if (ev.moves !== undefined) { if (ev.moves === "") { window.chessInitBoard(); window.chessMyTurn = window.chessIsWhite; } else { window.chessApplyMoves(ev.moves); } }
                if (ev.status && ev.status !== "started") { window.chessStatus = "ended"; window.chessResult = ev.status; window.saveChessState(); }
            }
        } catch(e) {}
    }
    function chessReconnectStream() {
        Quickshell.execDetached(["bash", "-c", "pkill -f 'chess_stream.sh.*" + window.chessGameId + "' 2>/dev/null; LICHESS_TOKEN=\"" + window.lichessToken.replace(/[\"\\]/g,"") + "\" ~/.config/hypr/scripts/quickshell/chess_stream.sh " + window.chessGameId.replace(/[^a-zA-Z0-9]/g,"") + " &"]);
        chessEventWatcher.running = false; chessEventWatcher.running = true;
    }
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            if (gameId) {
                ...
                Quickshell.execDetached(["bash", "-c",
                    "pkill -f 'chess_stream.sh.*' 2>/dev/null; LICHESS_TOKEN=\"" + window.lichessToken.replace(/[\"\\]/g,"") + "\" ~/.config/hypr/scripts/quickshell/chess_stream.sh " + gameId.replace(/[^a-zA-Z0-9]/g,"") + " &"
                ]);
    ...
    Process {
        id: chessEventWatcher; command: ["bash", "-c", "inotifywait -qq -e modify,close_write /tmp/qs_chess_event 2>/dev/null || sleep 60"]
        onExited: { chessEventReader.running = false; chessEventReader.running = true; running = false; running = true; }
    }
    Process {
        id: chessEventReader; command: ["bash", "-c", "echo idle"]
        stdout: StdioCollector { onStreamFinished: { if (this.text.trim() === "idle" || this.text.trim() === "") return; window.chessProcessEvent(this.text.trim()); } }
    }
    Connections { target: chessEventWatcher; function onExited() { chessEventReader.command = ["bash", "-c", "cat /tmp/qs_chess_event 2>/dev/null || echo idle"]; chessEventReader.running = false; chessEventReader.running = true; } }

    Component.onCompleted: { window.chessInitBoard(); window.loadCache("chat"); window.greetingFetched = true; }

    // ── Auto-save notes when the popup closes ──
    onVisibleChanged: {
        if (!visible && window.notesSubMode === "edit" && window.currentNoteFilepath !== "" && noteArea.text.trim() !== "") {
            autoSaveTimer.stop();
            let b64 = Qt.btoa(noteArea.text);
    ...
    Process { id: kavitaSeriesProc; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: {
        ...
        try { let data = JSON.parse(this.text.trim()); kavitaSeries.clear(); kavitaAllSeries.clear(); let arr = data || [];
            for (let i = 0; i < arr.length; i++) { let entry = { seriesId: arr[i].id || 0, name: arr[i].name || "Unknown", libraryName: arr[i].libraryName || "", libraryId: arr[i].libraryId || 0, pages: arr[i].pages || 0, pagesRead: arr[i].pagesRead || 0, format: arr[i].format || 0 };
                kavitaAllSeries.append(entry); if ((arr[i].libraryName || "").toLowerCase() === "textbooks") kavitaSeries.append(entry); } window.kavitaFetchOnDeck(); } catch(e) {}
    }}}
    ...
    Process {
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            try {
                ...
                window.kavitaReadPage = cp.pageNum || 0;
                if (window.kavitaReadChapterId > 0) {
                    window.kavitaLoadChapterInfo();
                } else {
                    // No continue point — fetch series detail to get first chapter
    ...
    Process {
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            try {
                ...
                // Flatten volumes → chapters
                for (let v = 0; v < volumes.length; v++) {
                    let vol = volumes[v];
                    let chs = vol.chapters || [];
                    for (let c = 0; c < chs.length; c++) {
                        let ch = chs[c];
                        kavitaChapters.append({
                            chapterId: ch.id, volumeId: vol.id,
                            title: ch.titleName || ch.range || ("Chapter " + ch.number),
                            pages: ch.pages || 0, pagesRead: ch.pagesRead || 0
                        });
                    }
                }
                // Also check specials/storyline chapters
                let specials = detail.chapters || [];
                for (let s = 0; s < specials.length; s++) {
                    let sp = specials[s];
                    kavitaChapters.append({
                        chapterId: sp.id, volumeId: sp.volumeId || 0,
                        title: sp.titleName || sp.range || ("Special " + (s+1)),
                        pages: sp.pages || 0, pagesRead: sp.pagesRead || 0
                    });
                }
                ...
                else { window.kavitaReadLoading = false; window.kavitaReadContent = "No chapters found."; }
            } catch(e) { window.kavitaReadLoading = false; window.kavitaReadContent = "Error loading series."; }
        }}
    }
    ...
    Process {
        ...
        stdout: StdioCollector { onStreamFinished: {
            ...
            try {
                ...
                window.kavitaReadTitle = info.title || info.chapterTitle || window.kavitaReadTitle;
                ...
                // Also fetch chapter list if we haven't yet
    ...
    // Image formats (0=manga/comics): use /api/Reader/image → loads into QML Image
    // Book formats (3=epub, 4=pdf): use /api/Book/{id}/book-page → loads HTML into Text
    function kavitaLoadPage(page) {
        window.kavitaReadLoading = true;
        window.kavitaReadPage = page;

        if (window.kavitaReadFormat === 3 || window.kavitaReadFormat === 4) {
            // Epub / PDF — fetch HTML content
    ...
    function kavitaDownloadBook(seriesId, name) {
        window.learnLoading = true; window.bookTitle = name;
        kavitaDownloadProc.command = ["bash", "-c", "LDIR=$HOME/.local/share/quickshell-learn && mkdir -p $LDIR && curl -s '" + window.kavitaUrl + "/api/Series/series-detail?seriesId=" + seriesId + "' -H 'Authorization: Bearer " + window.kavitaToken + "' 2>/dev/null"];
        kavitaDownloadProc.running = false; kavitaDownloadProc.running = true;
    }
    Process { id: kavitaDownloadProc; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { if (this.text.trim() === "idle") return; try { let detail = JSON.parse(this.text.trim()); let volumes = detail.volumes || []; let volId = -1; for (let i = 0; i < volumes.length; i++) { if (volumes[i].id) { volId = volumes[i].id; break; } }
        if (volId >= 0) { kavitaFileDownloader.command = ["bash", "-c", "LDIR=$HOME/.local/share/quickshell-learn && mkdir -p $LDIR && curl -s '" + window.kavitaUrl + "/api/Download/volume?volumeId=" + volId + "' -H 'Authorization: Bearer " + window.kavitaToken + "' -o $LDIR/kavita_book.download 2>/dev/null && MIME=$(file -b --mime-type $LDIR/kavita_book.download) && case $MIME in application/pdf) mv $LDIR/kavita_book.download $LDIR/kavita_book.pdf && echo $LDIR/kavita_book.pdf;; application/epub*) mv $LDIR/kavita_book.download $LDIR/kavita_book.epub && echo $LDIR/kavita_book.epub;; application/zip|application/x-cbz) mv $LDIR/kavita_book.download $LDIR/kavita_book.zip && mkdir -p $LDIR/extracted && cd $LDIR/extracted && unzip -o $LDIR/kavita_book.zip '*.txt' '*.html' '*.xhtml' 2>/dev/null && find $LDIR/extracted -type f \\( -name '*.txt' -o -name '*.html' -o -name '*.xhtml' \\) -exec cat {} + > $LDIR/kavita_book.txt && echo $LDIR/kavita_book.txt;; text/*) mv $LDIR/kavita_book.download $LDIR/kavita_book.txt && echo $LDIR/kavita_book.txt;; *) mv $LDIR/kavita_book.download $LDIR/kavita_book.pdf && echo $LDIR/kavita_book.pdf;; esac"];
            kavitaFileDownloader.running = false; kavitaFileDownloader.running = true; } else { window.learnLoading = false; } } catch(e) { window.learnLoading = false; } } } }
    Process { id: kavitaFileDownloader; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { if (this.text.trim() === "idle") return; let fp = this.text.trim(); if (fp && fp.length > 0) { window.processBook(fp); } else { window.learnLoading = false; } } } }

    function loadLearnConfig() {
        learnConfigLoader.command = ["bash", "-c", "LDIR=$HOME/.local/share/quickshell-learn && cat $LDIR/book_meta.json 2>/dev/null && echo '|||SPLIT|||' && cat $LDIR/progress.json 2>/dev/null"];
        learnConfigLoader.running = false; learnConfigLoader.running = true;
    }
    Process { id: learnConfigLoader; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { if (this.text.trim() === "idle") return; let parts = this.text.split("|||SPLIT|||");
        try { let meta = JSON.parse(parts[0].trim()); window.bookTitle = meta.title || ""; window.totalChapters = meta.total || 0; window.bookLoaded = meta.total > 0; bookChapters.clear();
            for (let i = 0; i < meta.chapters.length; i++) bookChapters.append({ title: meta.chapters[i].title, chIndex: meta.chapters[i].index, filepath: meta.chapters[i].filepath }); } catch(e) {}
        if (parts.length > 1) { try { let prog = JSON.parse(parts[1].trim()); window.currentChapter = prog.current_chapter || 0; learnedTerms.clear(); let vocab = prog.vocab || [];
            for (let i = 0; i < vocab.length; i++) { learnedTerms.append(vocab[i]); window.saveLearnState(); } } catch(e) {} } } } }
    ...
    function processBook(filepath) {
        ...
        bookProcessor.command = ["bash", "-c", "LDIR=$HOME/.local/share/quickshell-learn && mkdir -p $LDIR/chapters && python3 -c 'import re,json,os,sys\nfp=sys.argv[1]\nld=os.path.expanduser(\"~/.local/share/quickshell-learn\")\ncd=os.path.join(ld,\"chapters\")\nos.makedirs(cd,exist_ok=True)\next=fp.rsplit(\".\",1)[-1].lower()\nif ext==\"pdf\":\n import subprocess;r=subprocess.run([\"pdftotext\",fp,\"-\"],capture_output=True,text=True);text=r.stdout\nelse:\n text=open(fp,encoding=\"utf-8\",errors=\"replace\").read()\npats=[r\"(?m)^(?:Chapter|CHAPTER|Lesson|LESSON|Unit|UNIT|第)\\\\s*[\\\\d一二三四五六七八九十]+\",r\"(?m)^#{1,3}\\\\s+.+\",r\"(?m)^\\\\d+[\\\\.)\\\\s]+[A-Z].{5,}\"]\nsp=None\nfor p in pats:\n m=list(re.finditer(p,text))\n if len(m)>=2:sp=m;break\nchs=[]\nif sp and len(sp)>=2:\n for i,m in enumerate(sp):\n  s=m.start();e=sp[i+1].start() if i+1<len(sp) else len(text)\n  ch=text[s:e].strip()\n  if not ch:continue\n  t=ch.split(chr(10))[0].strip()[:80]\n  fn=f\"ch_{i:03d}.txt\"\n  open(os.path.join(cd,fn),\"w\").write(ch)\n  chs.append({\"title\":t,\"index\":i,\"filepath\":os.path.join(cd,fn)})\nelse:\n cs=3000\n for i in range(0,len(text),cs):\n  ch=text[i:i+cs].strip()\n  if not ch:continue\n  ix=i//cs;t=ch.split(chr(10))[0].strip()[:60] or f\"Section {ix+1}\"\n  fn=f\"ch_{ix:03d}.txt\"\n  open(os.path.join(cd,fn),\"w\").write(ch)\n  chs.append({\"title\":t,\"index\":ix,\"filepath\":os.path.join(cd,fn)})\nbt=os.path.basename(fp).rsplit(\".\",1)[0]\nmeta={\"title\":bt,\"source\":fp,\"chapters\":chs,\"total\":len(chs)}\njson.dump(meta,open(os.path.join(ld,\"book_meta.json\"),\"w\"))\npp=os.path.join(ld,\"progress.json\")\nif not os.path.exists(pp):json.dump({\"current_chapter\":0,\"vocab\":[]},open(pp,\"w\"))\nprint(json.dumps(meta))\n' '" + filepath.replace(/'/g, "'\\''") + "'"];
        bookProcessor.running = false; bookProcessor.running = true;
    }
    Process { id: bookProcessor; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { window.learnLoading = false; if (this.text.trim() === "idle") return;
        try { let meta = JSON.parse(this.text.trim().split("\n").pop()); window.bookTitle = meta.title || ""; window.totalChapters = meta.total || 0; window.bookLoaded = meta.total > 0; window.currentChapter = 0; bookChapters.clear();
            for (let i = 0; i < meta.chapters.length; i++) bookChapters.append({ title: meta.chapters[i].title, chIndex: meta.chapters[i].index, filepath: meta.chapters[i].filepath });
        } catch(e) { window.bookTitle = "Error loading book"; } } } }

    function loadChapter(index) { if (index < 0 || index >= bookChapters.count) return; let ch = bookChapters.get(index); window.currentChapter = index; window.currentChapterTitle = ch.title;
        chapterLoader.command = ["bash", "-c", "cat '" + ch.filepath.replace(/'/g, "'\\''") + "' 2>/dev/null"]; chapterLoader.running = false; chapterLoader.running = true; }
    Process { id: chapterLoader; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { if (this.text.trim() !== "idle") window.currentChapterContent = this.text; } } }

    function startLesson() {
        if (!window.bookLoaded) return; if (learnedTerms.count > 0) window.exportVocabToObsidian();
        window.learnSubMode = "lesson"; window.learnLoading = true; lessonChat.clear(); window.loadChapter(window.currentChapter); lessonStartTimer.restart();
    }
    Timer { id: lessonStartTimer; interval: 800; repeat: false; onTriggered: {
        let vocabList = []; for (let i = 0; i < learnedTerms.count; i++) { let t = learnedTerms.get(i); vocabList.push(t.term + " (" + t.meaning + ")"); }
        let sysPrompt = "You are a language tutor. The student is studying from a textbook. Here is the current chapter content:\\n\\n" + window.currentChapterContent.substring(0, 4000) + "\\n\\nThe student has already learned these terms: " + vocabList.join(", ") + "\\n\\nYour job: 1) Briefly introduce what this chapter covers. 2) Teach the key vocabulary and grammar points from this chapter. 3) Give practice exercises. 4) When the student speaks or types, evaluate their response. 5) Use the target language mixed with English explanations. 6) At the end of each response, list NEW vocabulary in this exact format on separate lines: VOCAB:term|reading|meaning (e.g. VOCAB:食べる|たべる|to eat). Be encouraging and conversational like Duolingo. Keep responses focused and not too long.";
        let payload = JSON.stringify({ model: window.selectedModel, stream: false, messages: [{ role: "system", content: sysPrompt }, { role: "user", content: "I'm ready to start this chapter. Please begin the lesson!" }] });
        let escaped = payload.replace(/'/g, "'\\''");
        learnApiCaller.command = ["bash", "-c", "echo '" + escaped + "' | curl -s -X POST '" + window.apiBaseUrl + "/api/chat/completions' -H 'Authorization: Bearer " + window.apiKey + "' -H 'Content-Type: application/json' -d @- 2>/dev/null"];
    ...
    function sendLearnMessage(text) {
        ...
        let msgs = [{ role: "system", content: "You are a language tutor continuing a lesson. Chapter content (abbreviated):\\n" + window.currentChapterContent.substring(0, 3000) + "\\nEvaluate student responses for correctness. Teach vocabulary and grammar from the chapter. End each response with any NEW vocab as VOCAB:term|reading|meaning lines. Be encouraging." }];
        for (let i = 0; i < lessonChat.count; i++) { let m = lessonChat.get(i); msgs.push({ role: m.role, content: m.content }); }
        let payload = JSON.stringify({ model: window.selectedModel, stream: false, messages: msgs }); let escaped = payload.replace(/'/g, "'\\''");
        learnApiCaller.command = ["bash", "-c", "echo '" + escaped + "' | curl -s -X POST '" + window.apiBaseUrl + "/api/chat/completions' -H 'Authorization: Bearer " + window.apiKey + "' -H 'Content-Type: application/json' -d @- 2>/dev/null"];
        learnApiCaller.running = false; learnApiCaller.running = true;
    }
    Process { id: learnApiCaller; command: ["bash", "-c", "echo idle"]; stdout: StdioCollector { onStreamFinished: { if (this.text.trim() === "idle") return; window.learnLoading = false;
        try { let data = JSON.parse(this.text.trim()); let resp = data.choices[0].message.content; let lines = resp.split("\n"); let cleanLines = [];
        ...
        } catch(e) { lessonChat.append({ role: "assistant", content: "Error: Could not get response." }); window.learnLastResponse = "Error: Could not get response."; window.learnTypeLen = 0; } } } }
...
anchors.verticalCenter: parent.verticalCenter }
            ...
            // Blobs
            Rectangle { width: parent.width * 0.8; height: width; radius: width / 2; x: parent.width / 2 - width / 2 + Math.cos(window.globalOrbitAngle * 2) * window.s(150); y: parent.height / 2 - height / 2 + Math.sin(window.globalOrbitAngle * 2) * window.s(100); opacity: 0.08; color: window.modeColor1
...
Behavior on color { ColorAnimation { duration: 500 } } }
            ...
            ColumnLayout {
                ...
                // ── Header ──
                RowLayout {
                    Layout.fillWidth: true; Layout.minimumHeight: window.s(36); Layout.maximumHeight: window.s(36); spacing: window.s(10); opacity: introTop
                    transform: Translate { y: window.s(-10) * (1.0 - introTop) }
                    Item { Layout.preferredWidth: window.s(80); Layout.fillHeight: true
                        Text { anchors.left: parent.left; anchors.verticalCenter: parent.verticalCenter
                            text: window.activeMode === "chess" ? "Lichess" : window.activeMode === "kavita" ? "Kavita" : window.activeMode === "chat" ? "AI" : window.activeMode === "notes" ? "Notes" : "Learn"
                            font.family: "JetBrains Mono"; font.weight: Font.Black; font.pixelSize: window.s(16); color: titleMa.containsMouse ? Qt.lighter(window.modeColor1, 1.2) : window.modeColor1
Behavior on color { ColorAnimation { duration: 200 } } }
                        MouseArea { id: titleMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: {
                    ...
                    Rectangle { Layout.preferredWidth: window.s(200); Layout.fillHeight: true; radius: window.s(10); color: window.surface0; border.color: window.surface1; border.width: 1
                        property var modes: ["chess", "kavita", "chat", "notes", "learn"]; property int modeIdx: Math.max(0, modes.indexOf(window.activeMode))
                        Rectangle { width: (parent.width - window.s(2)) / 5; height: parent.height - window.s(2); y: window.s(1); radius: window.s(8); x: parent.modeIdx * width + window.s(1)
Behavior on x { NumberAnimation { duration: 300; easing.type: Easing.OutBack; easing.overshoot: 1.1 } }
                            property color c1: window.activeMode === "chess" ? window.yellow : window.activeMode === "kavita" ? window.pink : window.activeMode === "chat" ? window.mauve : window.activeMode === "notes" ? window.peach : window.green
                            property color c2: window.activeMode === "chess" ? window.peach : window.activeMode === "kavita" ? window.mauve : window.activeMode === "chat" ? window.blue : window.activeMode === "notes" ? window.yellow : window.teal
...
Text { anchors.centerIn: parent; text: "󰐹"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.activeMode === "chess" ? "#ffffff" : window.overlay0
Behavior on color { ColorAnimation { duration: 200 } } }
MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: window.activeMode = "chess" } }
                            Item { Layout.fillWidth: true; Layout.fillHeight: true
Text { anchors.centerIn: parent; text: "󰀭"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.activeMode === "kavita" ? "#ffffff" : window.overlay0
Behavior on color { ColorAnimation { duration: 200 } } }
MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: { window.activeMode = "kavita"; if (window.kavitaConnected) window.kavitaFetchOnDeck(); else window.kavitaAuth(); } } }
                            Item { Layout.fillWidth: true; Layout.fillHeight: true
Text { anchors.centerIn: parent; text: "󰭻"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.activeMode === "chat" ? "#ffffff" : window.overlay0
Behavior on color { ColorAnimation { duration: 200 } } }
MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: window.activeMode = "chat" } }
                            Item { Layout.fillWidth: true; Layout.fillHeight: true
Text { anchors.centerIn: parent; text: "󰠮"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.activeMode === "notes" ? "#ffffff" : window.overlay0
Behavior on color { ColorAnimation { duration: 200 } } }
MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: { window.activeMode = "notes"; window.notesSubMode = "menu"; window.fetchVaultNotes(); } } }
                            Item { Layout.fillWidth: true; Layout.fillHeight: true
Text { anchors.centerIn: parent; text: "󰗊"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.activeMode === "learn" ? "#ffffff" : window.overlay0
Behavior on color { ColorAnimation { duration: 200 } } }
MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: { window.activeMode = "learn"; if (window.bookLoaded && window.learnSubMode === "home") { window.learnSubMode = "lesson"; window.startLesson(); } } } }
                ...
                Item { Layout.fillWidth: true; Layout.fillHeight: true; visible: window.activeMode === "chat"
                    ColumnLayout { anchors.fill: parent; spacing: window.s(8)
                        ...
                        // ── Top bar: temp indicator + New Chat ──
                        RowLayout { Layout.fillWidth: true; Layout.preferredHeight: window.s(28); spacing: window.s(8)
                            Item { Layout.fillWidth: true }
                            Text { visible: window.temporaryChat; text: "Temporary — not saved"; font.family: "JetBrains Mono"; font.pixelSize: window.s(9); color: window.peach; Layout.alignment: Qt.AlignVCenter }
                            Rectangle { Layout.preferredWidth: newChatRow.implicitWidth + window.s(16); Layout.preferredHeight: window.s(26); radius: window.s(8)
                                color: newChatMa.containsMouse ? Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.8) : Qt.rgba(window.surface0.r, window.surface0.g, window.surface0.b, 0.5)
                                border.color: newChatMa.containsMouse ? window.mauve : Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.5); border.width: 1
Behavior on color { ColorAnimation { duration: 120 } }
                                Row { id: newChatRow; anchors.centerIn: parent; spacing: window.s(6)
                                    ...
                                    Text { text: "New Chat"; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(10); color: newChatMa.containsMouse ? window.text : window.subtext0; anchors.verticalCenter: parent.verticalCenter } }
                                MouseArea { id: newChatMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { chatMessages.clear(); window.lastResponse = ""; window.typeLen = 0; window.temporaryChat = true; inputField.forceActiveFocus(); } } }
                        }
...
Behavior on opacity { NumberAnimation { duration: 600 } } } } }
                        ...
                        // Messages
                        ListView { id: chatView; Layout.fillWidth: true; Layout.fillHeight: true; visible: chatMessages.count > 0; model: chatMessages; clip: true; spacing: window.s(8); onCountChanged: positionViewAtEnd(); opacity: introChat
                            ...
                            delegate: Item { width: ListView.view ? ListView.view.width : 0; height: msgOuter.implicitHeight + window.s(8)
                                ColumnLayout { id: msgOuter; anchors.left: model.role === "assistant" ? parent.left : undefined; anchors.right: model.role === "user" ? parent.right : undefined; anchors.top: parent.top; width: Math.min(parent.width * 0.85, msgContent.implicitWidth + window.s(28)); spacing: window.s(4)
                                    ...
                                    Rectangle { Layout.fillWidth: true; Layout.preferredHeight: msgContent.implicitHeight + window.s(16); radius: window.s(12)
                                        ...
                                        Text { id: msgContent; anchors.left: parent.left; anchors.right: parent.right; anchors.top: parent.top; anchors.margins: model.role === "user" ? window.s(10) : window.s(4); anchors.topMargin: model.role === "user" ? window.s(8) : window.s(4)
                                            text: { if (model.role === "assistant" && index === chatMessages.count - 1) return window.displayedResponse; return model.content; }
                                            font.family: "JetBrains Mono"; font.pixelSize: window.s(12); color: window.text; wrapMode: Text.Wrap; textFormat: Text.PlainText; lineHeight: 1.4 } } } } }
...
Behavior on color { ColorAnimation { duration: 80 } }
                                        ...
                                        MouseArea { id: backMenuMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { window.notesSubMode = "menu"; window.fetchVaultNotes(); } } }
                                    Text { text: window.selectedNoteTitle; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(13); color: window.text; elide: Text.ElideRight; Layout.fillWidth: true }
...
Behavior on opacity { NumberAnimation { duration: 300 } } }
                                    Rectangle { Layout.preferredWidth: aiExpandRow.implicitWidth + window.s(16); Layout.preferredHeight: window.s(28); radius: window.s(8)
                                        color: aiExpandMa.containsMouse ? Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.8) : "transparent"; border.color: aiExpandMa.containsMouse ? window.mauve : Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.5); border.width: 1
Behavior on color { ColorAnimation { duration: 120 } }
...
Text { text: "Expand with AI"; font.family: "JetBrains Mono"; font.pixelSize: window.s(10); color: aiExpandMa.containsMouse ? window.text : window.subtext0; anchors.verticalCenter: parent.verticalCenter } }
                                        MouseArea { id: aiExpandMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: { if (noteArea.text.trim() === "") return; window.activeMode = "chat"; window.sendMessage("Expand and improve these notes into a well-structured document. Keep the original ideas but add detail, fix grammar, and organize with headers:\n\n" + noteArea.text); } } } }
                                Rectangle { Layout.fillWidth: true; Layout.fillHeight: true; radius: window.s(10); color: Qt.rgba(window.base.r, window.base.g, window.base.b, 0.65); border.color: noteArea.activeFocus ? window.peach : Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.6); border.width: noteArea.activeFocus ? 2 : 1
Behavior on border.color { ColorAnimation { duration: 200 } }
                                    TextEdit { id: noteArea; anchors.fill: parent; anchors.margins: window.s(10); font.family: "JetBrains Mono"; font.pixelSize: window.s(12); color: window.text; wrapMode: TextEdit.Wrap; selectByMouse: true; selectionColor: Qt.rgba(window.peach.r, window.peach.g, window.peach.b, 0.3)
...
Text { visible: model.pages > 0; text: model.pages + " pages"; font.family: "JetBrains Mono"; font.pixelSize: window.s(8); color: window.overlay0 } } }
                                                        MouseArea { id: kavSeriesMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: window.kavitaDownloadBook(model.seriesId, model.name) } } }
...
Text { text: model.role === "user" ? "You" : "Tutor"; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(10); color: model.role === "user" ? window.blue : window.green } }
                                            Text { text: { if (model.role === "assistant" && index === lessonChat.count - 1) return window.learnDisplayedResponse; return model.content; }
font.family: "JetBrains Mono"; font.pixelSize: window.s(11); color: window.text; Layout.fillWidth: true; wrapMode: Text.Wrap; textFormat: Text.PlainText; lineHeight: 1.4 } } } }
...
Text { text: model.meaning; font.family: "JetBrains Mono"; font.pixelSize: window.s(10); color: window.overlay1; elide: Text.ElideRight; Layout.fillWidth: true } } } } }
                    ...
                    ColumnLayout { anchors.fill: parent; spacing: window.s(6); opacity: introChat
                        ...
                        Item { Layout.fillWidth: true; Layout.fillHeight: true
                            ...
                            ColumnLayout {
                                ...
                                // Human games — 3x1 grid of square-ish buttons
                                Text {
                                    visible: window.lichessToken !== ""
                                    text: "Play Online"
                                ...
                                RowLayout {
                                    ...
                                    Repeater {
                                        model: ListModel {
                                            ListElement { label: "1+0"; desc: "Bullet"; mins: 1 }
                                            ListElement { label: "3+2"; desc: "Blitz"; mins: 3 }
                                            ListElement { label: "10+0"; desc: "Rapid"; mins: 10 }
                                        }
                                        delegate: Rectangle {
                                            ...
                                            ColumnLayout {
                                                ...
                                                Text {
                                                    ...
                                                    font.pixelSize: window.s(16); color: hgMa.containsMouse ? window.yellow : window.text
                                            ...
                                            MouseArea {
                                                ...
                                                onClicked: window.chessSeekGame(mins)
                                            }
                                        }
                                    }
                                }
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    Slider {
                                        ...
                                        value: window.chessAiElo
                                        onValueChanged: window.chessAiElo = value

                                        background: Rectangle {
                                            x: eloSlider.leftPadding; y: eloSlider.topPadding + eloSlider.availableHeight / 2 - height / 2
                                            width: eloSlider.availableWidth; height: window.s(4); radius: window.s(2)
                                        ...
                                        handle: Rectangle {
                                            ...
                                            width: window.s(16); height: window.s(16); radius: window.s(8)
                                ...
                                Rectangle {
                                    ...
                                    Row {
                                        ...
                                        Text { text: "󰐊"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(18); color: window.crust; anchors.verticalCenter: parent.verticalCenter }
                                        Text {
                                            text: "Play (Level " + window.chessEloToLevel(window.chessAiElo) + ")"
                                            font.family: "JetBrains Mono"; font.weight: Font.Black
                                            font.pixelSize: window.s(13); color: window.crust
                                            anchors.verticalCenter: parent.verticalCenter
                                        }
                                    }
                                    MouseArea { id: aiPlayMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: window.chessStartAiFromElo() }
                                }
                            }
                        }
                        ...
                        Item { Layout.fillWidth: true; Layout.fillHeight: true
                            ...
                            ColumnLayout { anchors.fill: parent; spacing: window.s(4)
                                ...
                                // Puzzle board (reuses the same board rendering)
                                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: width; Layout.maximumHeight: parent.height - window.s(50)
                                    radius: window.s(8); color: window.crust; clip: true; border.color: window.surface1; border.width: 1
                                    Grid { anchors.fill: parent; anchors.margins: window.s(3); columns: 8; rows: 8
                                        Repeater { model: 64
                                            delegate: Rectangle {
                                                ...
                                                color: isSelected ? Qt.rgba(window.yellow.r, window.yellow.g, window.yellow.b, 0.5) : isLight ? Qt.rgba(window.text.r, window.text.g, window.text.b, 0.15) : Qt.rgba(window.surface0.r, window.surface0.g, window.surface0.b, 0.6)
                                                Text { anchors.centerIn: parent; text: window.chessPieceChar(piece); font.pixelSize: parent.width * 0.7; color: piece >= "A" && piece <= "Z" ? "#ffffff" : "#1a1a2e"; style: Text.Outline; styleColor: piece >= "A" && piece <= "Z" ? "#333333" : "#aaaaaa" }
                                            }
                                        }
                                    }
                                }
                                Text { text: (window.chessIsWhite ? "White" : "Black") + " to move"; font.family: "JetBrains Mono"; font.pixelSize: window.s(10); color: window.overlay0; Layout.alignment: Qt.AlignHCenter }
                            }
                        }
                        ...
                        Item { Layout.fillWidth: true; Layout.fillHeight: true
                            ...
                            ColumnLayout { anchors.fill: parent; spacing: window.s(4)
                                ...
                                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: width; Layout.maximumHeight: parent.height - window.s(60)
                                    radius: window.s(8); color: window.crust; clip: true; border.color: window.surface1; border.width: 1
                                    Grid { anchors.fill: parent; anchors.margins: window.s(3); columns: 8; rows: 8
                                        Repeater { model: 64
                                            delegate: Rectangle {
                                                ...
                                                color: isSelected ? Qt.rgba(window.yellow.r, window.yellow.g, window.yellow.b, 0.5) : (isLastFrom || isLastTo) ? Qt.rgba(window.blue.r, window.blue.g, window.blue.b, 0.25) : isLight ? Qt.rgba(window.text.r, window.text.g, window.text.b, 0.15) : Qt.rgba(window.surface0.r, window.surface0.g, window.surface0.b, 0.6)
                                                Text { anchors.centerIn: parent; text: window.chessPieceChar(piece); font.pixelSize: parent.width * 0.7; color: piece >= "A" && piece <= "Z" ? "#ffffff" : "#1a1a2e"; style: Text.Outline; styleColor: piece >= "A" && piece <= "Z" ? "#333333" : "#aaaaaa" }
                                                MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; enabled: window.chessStatus === "playing"; onClicked: window.chessSquareClicked(index) }
                                            }
                                        }
                                    }
                                }
                                RowLayout { Layout.fillWidth: true; spacing: window.s(6)
                                    ...
                                    Item { Layout.fillWidth: true }
                                    Text { visible: window.chessStatus === "ended"; text: window.chessResult; font.family: "JetBrains Mono"; font.weight: Font.Black; font.pixelSize: window.s(12); color: window.chessResult === "mate" || window.chessResult === "resign" ? window.red : window.yellow }
                                    Rectangle { visible: window.chessStatus === "playing"; Layout.preferredWidth: resignRow.implicitWidth + window.s(14); Layout.preferredHeight: window.s(26); radius: window.s(6)
                                        color: resignMa.containsMouse ? window.surface1 : "transparent"
                                        border.color: resignMa.containsMouse ? window.red : "transparent"; border.width: 1
                                        Row { id: resignRow; anchors.centerIn: parent; spacing: window.s(4)
                                    ...
                                    Rectangle { visible: window.chessStatus === "ended"; Layout.preferredWidth: newGameRow.implicitWidth + window.s(14); Layout.preferredHeight: window.s(26); radius: window.s(6)
                                        ...
                                        MouseArea { id: newGameMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                                            onClicked: { window.chessStatus = "menu"; window.chessGameId = ""; window.chessSelected = -1; window.chessFromIdx = -1; window.chessToIdx = -1; window.chessInitBoard(); chessStreamProc.running = false; chessEventWatcher.running = false; Quickshell.execDetached(["bash", "-c", "pkill -f chess_stream.sh 2>/dev/null"]); window.saveChessState(); } } }
                                }
                            }
                        }
                    }
                ...
                Item { Layout.fillWidth: true; Layout.fillHeight: true; visible: window.activeMode === "kavita"
                    ColumnLayout { anchors.fill: parent; spacing: window.s(8); opacity: introChat
                        ...
                        // ── Top bar (shown when NOT reading) ──
                        RowLayout { Layout.fillWidth: true; spacing: window.s(8)
                            ...
                            Item { Layout.fillWidth: true }
                            Rectangle { visible: window.kavitaConnected; Layout.preferredWidth: window.s(130); Layout.preferredHeight: window.s(28); radius: window.s(8); color: window.surface0; border.color: window.surface1; border.width: 1
                                Rectangle { width: parent.width / 2 - window.s(1); height: parent.height - window.s(2); y: window.s(1); radius: window.s(6)
                        ...
                        Item { Layout.fillWidth: true; Layout.fillHeight: true
                            ...
                            ColumnLayout { anchors.fill: parent; spacing: window.s(6)
                                RowLayout { Layout.fillWidth: true; spacing: window.s(4)
                                    Repeater { model: ["All", "Textbooks", "Novels", "Manga"]
                                        delegate: Rectangle { Layout.fillWidth: true; Layout.preferredHeight: window.s(26); radius: window.s(6); property string fv: modelData.toLowerCase()
                                ...
                                ListView { Layout.fillWidth: true; Layout.fillHeight: true; clip: true; spacing: window.s(4); boundsBehavior: Flickable.StopAtBounds; model: kavitaAllSeries
                                    ScrollBar.vertical: ScrollBar { width: window.s(3); policy: ScrollBar.AsNeeded; contentItem: Rectangle { implicitWidth: window.s(3); radius: window.s(1); color: window.surface2 } }
                                    delegate: Rectangle { width: ListView.view ? ListView.view.width : 0
...
Behavior on color { ColorAnimation { duration: 100 } }
                                        ColumnLayout { id: lsCol; anchors.left: parent.left; anchors.right: parent.right; anchors.top: parent.top; anchors.margins: window.s(8); spacing: window.s(3)
                                            ...
                                            RowLayout { Layout.fillWidth: true; spacing: window.s(8)
                                                ...
                                                Rectangle { visible: model.pagesRead > 0; Layout.fillWidth: true; height: window.s(3); radius: window.s(1); color: window.surface1
                                                    Rectangle { width: model.pages > 0 ? parent.width * (model.pagesRead / model.pages) : 0; height: parent.height; radius: parent.radius; color: window.pink } }
                                        ...
                                        MouseArea { id: lsMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                                            onClicked: window.kavitaOpenSeries(model.seriesId, model.libraryId, model.name, model.format) } } }
                        ...
                        Item { Layout.fillWidth: true; Layout.fillHeight: true
                            ...
                            ColumnLayout { anchors.fill: parent; spacing: window.s(6)
                                ...
                                // Reader header: back + title + open-in-browser
                                RowLayout { Layout.fillWidth: true; spacing: window.s(6)
                                    Rectangle { Layout.preferredWidth: window.s(28); Layout.preferredHeight: window.s(28); radius: window.s(8)
                                        color: rdrBackMa.containsMouse ? Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.8) : "transparent"
                                        Behavior on color { ColorAnimation { duration: 80 } }
                                        Text { anchors.centerIn: parent; text: "󰁍"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(15); color: window.pink }
                                        MouseArea { id: rdrBackMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                                            onClicked: { window.kavitaSaveProgress(); window.kavitaSubMode = "ondeck"; window.kavitaFetchOnDeck(); } } }
                                    ColumnLayout { Layout.fillWidth: true; spacing: 0
                                        Text { text: window.kavitaReadTitle; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(11); color: window.text; elide: Text.ElideRight; Layout.fillWidth: true }
                                        Text { text: "Page " + (window.kavitaReadPage + 1) + " / " + window.kavitaReadTotalPages; font.family: "JetBrains Mono"; font.pixelSize: window.s(8); color: window.overlay0 } }
                                    // Open in browser
                                    Rectangle { Layout.preferredWidth: window.s(28); Layout.preferredHeight: window.s(28); radius: window.s(8)
                                        color: rdrBrowserMa.containsMouse ? Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.8) : "transparent"
                                        Text { anchors.centerIn: parent; text: "󰀭"; font.family: "Iosevka Nerd Font"; font.pixelSize: window.s(14); color: window.overlay1 }
                                        MouseArea { id: rdrBrowserMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: window.kavitaOpenInBrowser() } }
                                }

                                // Progress bar
                                Rectangle { Layout.fillWidth: true; Layout.preferredHeight: window.s(3); radius: window.s(1); color: window.surface1
...
GradientStop { position: 1.0; color: window.mauve } }
                                ...
                                // Content area — image for manga/comics, rich text for epub/pdf
                                // Image reader (manga, comics, archives)
                                Flickable { id: readerImageFlick
                                    Layout.fillWidth: true; Layout.fillHeight: true
                                    visible: !window.kavitaReadLoading && window.kavitaReadFormat !== 3 && window.kavitaReadFormat !== 4
                                    contentWidth: width; contentHeight: readerImage.paintedHeight > 0 ? readerImage.paintedHeight : height
                                    ...
                                    Image {
                                        ...
                                        // Tap left/right halves to navigate pages
                                        MouseArea {
                                            anchors.fill: parent
                                ...
                                // Book reader (epub, pdf — HTML content)
                                Flickable { id: readerFlick
                                    Layout.fillWidth: true; Layout.fillHeight: true
                                    visible: !window.kavitaReadLoading && (window.kavitaReadFormat === 3 || window.kavitaReadFormat === 4)
                                ...
                                // Page navigation: [prev] [chapter list] [next]
